...
class EmoIdBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        wavs_sp, lens_sp = batch.sig_sp
        wavs_voc, lens_voc = batch.sig_voc

        #outputs_sp = self.compute_outputs(wavs_sp, lens_sp)
        outputs_voc = self.modules.wav2vec2(wavs_voc)
        outputs_sp = outputs_voc

        outputs_sp = self.global_average(self.modules.sp_cnn(outputs_sp), lens_voc)
        outputs_chn = self.global_average(self.modules.chn_cnn(outputs_voc), lens_voc)
        outputs_fan = self.global_average(self.modules.fan_cnn(outputs_voc), lens_voc)
        outputs_man = self.global_average(self.modules.man_cnn(outputs_voc), lens_voc)
        # last dim will be used for AdaptativeAVG pool

        outputs_sp = self.modules.output_mlp_sp(outputs_sp)
        outputs_chn = self.modules.output_mlp_chn(outputs_chn)
        outputs_fan = self.modules.output_mlp_fan(outputs_fan)    
        outputs_man = self.modules.output_mlp_man(outputs_man)

        return outputs_sp, outputs_chn, outputs_fan, outputs_man

    def global_average(self, outputs, lens):
        # last dim will be used for AdaptativeAVG pool
...
def dataio_prep(hparams):
    ...
    @sb.utils.data_pipeline.takes("wav_sp")
    @sb.utils.data_pipeline.provides("sig_sp")
    def audio_pipeline_sp(wav):
    ...
    @sb.utils.data_pipeline.provides("sig_voc")
    def audio_pipeline_voc(wav):
        """Load the signal, and pass it and its length to the corruption class.
        This is done on the CPU in the `collate_fn`."""
        sig = sb.dataio.dataio.read_audio(wav)
        return sig
    ...
    def label_pipeline_sp(input):
        ...
        dict_map={"CHN":1,"FAN":2,"MAN":3,"CXN":4,
                    "CHN_BG":5,"FAN_BG":6,"MAN_BG":7,"CXN_BG":8,"SIL":0}
    ...
    for dataset in ["train", "valid", "test"]:
        datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_json(
            ...
            dynamic_items=[audio_pipeline_sp, audio_pipeline_voc, label_pipeline_sp, label_pipeline_chn, label_pipeline_fan, label_pipeline_man],
            output_keys=["id", "sig_sp", "sig_voc","sp_true", "chn_true", "fan_true", "man_true"],
        )
