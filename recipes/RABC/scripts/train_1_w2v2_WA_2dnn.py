...
class EmoIdBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        outputs_adu = self.get_wa_outputs(outputs_adu,self.modules.weighted_average_adu,self.modules.dnn_adu,self.modules.output_mlp_adu, mean_pool_first=self.hparams.mean_pool_first_adu, lens = lens_adu)
        outputs_chi = self.get_wa_outputs(outputs_chi,self.modules.weighted_average_chi,self.modules.dnn_chi,self.modules.output_mlp_chi, mean_pool_first=self.hparams.mean_pool_first_chi, lens = lens_chi)

        return outputs_adu, outputs_chi
    
    def get_wa_outputs(self, outputs, wa, dnn, mlp, mean_pool_first=True, lens=None):
        if mean_pool_first:
            avg_outputs = []
