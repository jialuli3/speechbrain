...
class KICMultitaskBinaryMetricStats(MetricStats):
    ...
    def __init__(self,record_man=True):
        self.clear()
        self.man=record_man # whether to separate ADU classification into MAN or FAN
    ...
    def summarize_helper(self, preds, labels, curr_type=""):
        self.summary["accuracy{}".format(curr_type)]=round(accuracy_score(labels,preds),3)
        self.summary["macro_f1{}".format(curr_type)]=round(f1_score(labels,preds,average="macro"),3)
        self.summary["kappa{}".format(curr_type)]=round(cohen_kappa_score(labels,preds),3)
        self.summary["confusion_matrix{}".format(curr_type)]=confusion_matrix(labels,preds)
